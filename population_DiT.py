...
class PopulationDiT(nn.Module):
    ...
    def __init__(self, 
        ...
        ## FinalLayer
        self.finallayer = FinalLayer(self.hidden_size, 1 + sum(person_categorical_dims))
        # 个人变量投影
        self.person_continuous_proj = nn.Linear(1 + sum(person_categorical_dims), hidden_size)
    ...
    def forward(self, family_data, person_data, t):
        ...
        person_final_out = self.forward_person(person_data, node_embeddings, family_final_out, t, None)

        
        return family_final_out, person_final_out, hgt_data

    def forward_family(self, family_embedding_t, t_family):
        # 编码家庭信息
        ...
        for block in self.family_blocks:
            sequence = block(family_cont_embed, t_embed)
        
        # 输出
        sequence = self.output_norm_family(sequence)
        output = self.output_proj_family(sequence.flatten(start_dim=1))
    ...
    def forward_person(self, person_embedding_t, node_information, family_feature, t_person, person_mask):
        # 编码个人信息
        person_cont_embed = self.person_continuous_proj(person_embedding_t)
        
        # 时间步嵌入
        t_embed = self.time_embed(self.timestep_embedding(t_person, self.hidden_size))
