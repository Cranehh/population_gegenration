...
class ChemicalDetails:
    def __init__(self):
        self.atom_types = ['Male_Child', 'Female_Child', 'Male_Adult', 'Female_Adult', 'Male_Elder', 'Female_Elder']
        # self.bond_types = [Chem.BondType.SINGLE, Chem.BondType.DOUBLE, Chem.BondType.TRIPLE, Chem.BondType.AROMATIC]
        self.bond_types = ['SPOUSE', 'PARENT_CHILD', 'SIBLING', 'GRANDPARENT_GRANDCHILD', 'EXTENDED_FAMILY']
        # For family graphs, we'll define string-based relation types
        self.family_relation_types = ['SPOUSE', 'PARENT_CHILD', 'SIBLING', 'GRANDPARENT_GRANDCHILD', 'EXTENDED_FAMILY']
    ...
    def setup(self):
        ...
        self.edge_to_idx = {k:v for v,k  in enumerate(self.family_relation_types)}
    ...
    def set_edge_types(self, new_edge_types):
        self.bond_types = new_edge_types
        print(f"New edge types are {self.bond_types}")
        self.setup()
    ...
    def set_for_dataset(self, dataset: str):
        if dataset == "zinc":
            ...
            # Family member node types - you can choose different schemes:
            
            # Option 1: By role (recommended for family structure analysis)
            # self.set_atom_types(['Head', 'Spouse', 'Child', 'Parent', 'Grandparent', 'Other'])
            
            # Option 2: By age-gender (uncomment to use)
            self.set_atom_types(['Male_Child', 'Female_Child', 'Male_Adult', 'Female_Adult', 'Male_Elder', 'Female_Elder'])
            
            # Option 3: By occupation status (uncomment to use)  
            # self.set_atom_types(['Student', 'Employed', 'Unemployed', 'Retired', 'Homemaker', 'Child'])
            
            # Family relationship edge types
            self.set_edge_types(['SPOUSE', 'PARENT_CHILD', 'SIBLING', 'GRANDPARENT_GRANDCHILD', 'EXTENDED_FAMILY'])
        else:
