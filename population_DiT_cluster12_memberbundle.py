...
class PopulationDiT(nn.Module):
    ...
    def __init__(self, 
                 family_continuous_dim=8,
                 family_categorical_dims=[2],  # have_student, 家庭年收入类别数
        ...
        # 家庭变量投影
        self.family_continuous_proj = nn.Linear(8 + sum(family_categorical_dims), hidden_size)
        
        ## 家庭关系图生成
        # self.relation_graph_decoder = Decoder(8, 8 + sum(family_categorical_dims), True)
        ## 家庭关系图处理
        ...
        ## 家庭条件信息投影
        self.family_condition_proj = nn.Linear(8 + sum(family_categorical_dims), hidden_size)
        ## FinalLayer
        self.finallayer_family = FinalLayer(self.hidden_size, 8 + sum(family_categorical_dims))
        ...
        self.profile_proj_family = nn.Sequential(
            nn.Linear(69, hidden_size),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size),
        )

        self.profile_proj_person = nn.Sequential(
            nn.Linear(69, hidden_size),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size),
        )
        ...
        self.cluster_norm_family = nn.LayerNorm(hidden_size)
        self.cluster_norm_person = nn.LayerNorm(hidden_size)



        
        self.person_embedding_fusion = nn.LayerNorm(hidden_size)
    ...
    def forward(self, family_data, person_data, cluster, cluster_profile, person_mask, t, t_person):
        """
        ...
        # 准备输入
        family_final_out = self.forward_family(family_data, cluster, cluster_profile, t)
        ...
        person_final_out = self.forward_person(person_data, family_final_out, t_person, person_mask, cluster, cluster_profile)

        
        return family_final_out, person_final_out, None

    def forward_family(self, family_embedding_t, cluster, cluster_profile, t_family):
        # 编码家庭信息
        ...
        profile_embed = self.profile_proj_family(cluster_profile)
    ...
    def reference_family(self, family_embedding_t, t_family, cluster):
        # 编码家庭信息
        family_cont_embed = self.family_continuous_proj(family_embedding_t)
        family_cont_embed = family_cont_embed.unsqueeze(1)  # [b, 1, h]
        ...
        output = sequence.flatten(start_dim=1)
        ## softmax分类输出
        family_have_student_out = self.family_have_student_class(
            output[:, self.family_continuous_dim:self.family_continuous_dim + self.family_categorical_dims[0]])
        family_income_out = self.family_income_class(output[:,
                                                     self.family_continuous_dim + self.family_categorical_dims[
                                                         0]:self.family_continuous_dim + sum(
    ...
    def reference_person(self, person_embedding_t, t_person, family_feature, person_mask, family_cluster):

        self.relation_graph_decoder.update(family_feature)
        ...
        # 提取家庭成员关系图特征
        node_information = self.hgt_model(hgt_data)
        node_information = self.node_embedding_layer(node_information.view(node_information.shape[0], -1))

        B, P, N = person_embedding_t.shape
        person_mask = person_mask.view(-1)
        person_embedding_t = person_embedding_t.view(B * P, -1)[person_mask]
        # 编码个人信息
        person_cont_embed = self.person_continuous_proj(person_embedding_t)

        person_all_embed = torch.zeros(B * P, person_cont_embed.shape[-1]).to(person_embedding_t.device)
        person_all_embed[person_mask] = person_cont_embed
        person_all_embed = person_all_embed.view(B, P, -1)
        ...
        cross_attn_fusion = self.person_embedding_fusion(node_information + family_feature)
        cross_attn_fusion = cross_attn_fusion.unsqueeze(1)
        # Transformer处理
        for block in self.person_blocks:
            sequence = block(person_all_embed, cross_attn_fusion, cluster_embed, person_mask)
        family_feature = family_feature.squeeze(1)
        # 输出
        # output = self.finallayer(sequence, family_feature)
        output = self.finallayer2(sequence, cluster_embed)

        output = output.view(B * P, -1)
        output = output[person_mask]

        ## softmax分类输出
        person_gender_out = self.person_gender_class(
            output[:, self.person_continuous_dim:self.person_continuous_dim + self.person_categorical_dims[0]])
        person_license_out = self.person_license_class(output[:,
                                                       self.person_continuous_dim + self.person_categorical_dims[
                                                           0]:self.person_continuous_dim + sum(
                                                           self.person_categorical_dims[:2])])
        person_relation_out = self.person_relation_class(output[:, self.person_continuous_dim + sum(
            self.person_categorical_dims[:2]):self.person_continuous_dim + sum(self.person_categorical_dims[:3])])
        person_education_out = self.person_education_class(output[:, self.person_continuous_dim + sum(
            self.person_categorical_dims[:3]):self.person_continuous_dim + sum(self.person_categorical_dims[:4])])
        person_occupation_out = self.person_occupation_class(output[:, self.person_continuous_dim + sum(
            self.person_categorical_dims[:4]):self.person_continuous_dim + sum(self.person_categorical_dims)])
        ...
        person_all_embed = torch.zeros(B * P, N).to(person_embedding_t.device)
        person_all_embed[person_mask] = person_final_out
        person_final_out = person_all_embed.view(B, P, -1)

        return person_final_out, hgt_data
    
    def reference_all(self, x, t, cluster, cluster_profile, **kwargs):

        family_embedding_t, person_embedding_t = x
        ...
        # 时间步嵌入
        t_embed = self.time_embed(self.timestep_embedding(t, self.hidden_size))
        ...
        family_final_result = torch.cat([output[:, :self.family_continuous_dim], 
                                      family_have_student_out, 
                                      family_income_out], dim=-1)

        family_feature = family_final_result.clone()
        valid_person = family_feature[:, 0]
        valid_person = torch.round(valid_person * 0.88397094 + 2.38862088)
        person_mask = torch.zeros(family_feature.shape[0], 8, device=family_feature.device).to(torch.bool)
        ...
        # 时间步嵌入
        t_embed = self.person_time_embed(self.timestep_embedding(t, self.hidden_size))

        cluster_embed = self.y_embedder_person(cluster, self.training)
        profile_embed = self.profile_proj_person(cluster_profile)
        cluster_embed = cluster_embed + profile_embed
        cluster_embed = self.cluster_norm_person(cluster_embed)
        cluster_embed = cluster_embed + t_embed
        # node_information = node_information + t_embed
        family_feature = self.family_condition_proj(family_feature) + t_embed
        cross_attn_fusion = self.person_embedding_fusion(family_feature)
        cross_attn_fusion = cross_attn_fusion.unsqueeze(1)
        # Transformer处理
        for block in self.person_blocks:
            sequence = block(person_all_embed, cross_attn_fusion, cluster_embed, person_mask)
        ...
        return family_final_result, person_final_out, None
    

    def forward_person(self, person_embedding_t, family_feature, t_person, person_mask, family_cluster, cluster_profile):
        B, P, N = person_embedding_t.shape
        person_mask = person_mask.view(-1)
        person_embedding_t = person_embedding_t.view(B*P, -1)[person_mask]
        # 编码个人信息
...
def load_population_dit_checkpoint(checkpoint_path, device='cuda'):
    ...
    # 提取模型参数
    family_continuous_dim = getattr(args, 'family_continuous_dim', 8)
    family_categorical_dims = getattr(args, 'family_categorical_dims', [2])
