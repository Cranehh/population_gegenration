...
def compute_person_losses(person_pred, person_true, person_mask, family_pred):
    ...
    ## total member loss
    pred_person_number_mask = torch.sum(pred_person_mask, dim=-1)
    losses['total_member_loss'] = F.mse_loss(pred_person_number_mask, family_pred[:, 0] * 0.88413577 + 2.38879677, reduction='none')
