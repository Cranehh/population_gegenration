...
def multi_scale_guide_function(x, t, grid_person_num, current_grid_distribution_test, grid_control_test,
    ...
    return family_gradient, torch.clip(person_gradient, min=-1.0, max=1)
...
for district in district_control.keys():
    ...
    for grid in range(start, len(grid_control_district)):
        ...
        grid_family.to_csv(f'生成数据_newguidance/人口栅格_{district}_栅格{grid}_家庭数据.csv', index=False)
        grid_person.to_csv(f'生成数据_newguidance/人口栅格_{district}_栅格{grid}_个人数据.csv', index=False)
