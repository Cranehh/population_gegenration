...
def cagrad(grads_list, c=0.5, rescale=1):
    """
    Conflict-Averse Gradient descent (CAGrad)
    
    参考论文: Conflict-Averse Gradient Descent for Multi-task Learning (NeurIPS 2021)
    
    Args:
        grads_list: list of gradients, 每个元素shape相同
        c: 控制冲突规避强度的参数，越大越保守 (0, 1]
        rescale: 是否rescale最终梯度 (0: 不rescale, 1: rescale到平均梯度范数)
    ...
    valid_grads = []
    for g in grads_list:
        if g is not None and g.abs().sum() > 1e-10:
            valid_grads.append(g)
    
    if len(valid_grads) == 0:
        return torch.zeros_like(grads_list[0])
    
    if len(valid_grads) == 1:
        return valid_grads[0]
    
    # 展平所有梯度
    grads_flat = torch.stack([g.flatten().float() for g in valid_grads])  # [num_tasks, dim]
    num_tasks = grads_flat.shape[0]
    
    # 计算平均梯度
    g_avg = grads_flat.mean(dim=0)
    ...
    # 计算各任务梯度与平均梯度的内积
    dots = torch.matmul(grads_flat, g_avg)  # [num_tasks]
    
    # 如果所有内积都非负，说明没有冲突
    if dots.min() >= 0:
        result = g_avg.view_as(grads_list[0])
        if rescale:
            result = result * (sum(g.norm() for g in valid_grads) / num_tasks / (result.norm() + 1e-10))
        return result
    
    # 有冲突时，求解二次规划问题
    # 目标: min ||g_0 + sum(w_i * (g_i - g_0))||^2
    # 约束: sum(w_i) <= c, w_i >= 0
    
    # 构建Gram矩阵: G_ij = <g_i - g_0, g_j - g_0>
    g_diff = grads_flat - g_avg.unsqueeze(0)  # [num_tasks, dim]
    GG = torch.matmul(g_diff, g_diff.t())  # [num_tasks, num_tasks]
    
    # g0_diff = <g_0, g_i - g_0>
    g0_diff = torch.matmul(g_diff, g_avg)  # [num_tasks]
    
    # 使用Frank-Wolfe算法求解
    w = torch.zeros(num_tasks, device=grads_flat.device, dtype=grads_flat.dtype)
    
    for _ in range(20):  # Frank-Wolfe迭代
        # 计算梯度: grad_w = 2 * GG @ w + 2 * g0_diff
        grad_w = 2 * torch.matmul(GG, w) + 2 * g0_diff
        
        # 找到最小梯度分量对应的索引
        min_idx = grad_w.argmin()
        
        # 如果当前点已经是最优，退出
        if grad_w[min_idx] >= 0 and w.sum() <= c:
            break
        ...
        # 线搜索找到最优步长
        d = s - w
        dGd = torch.dot(d, torch.matmul(GG, d))
        dg0 = torch.dot(d, g0_diff)
        wGd = torch.dot(w, torch.matmul(GG, d))
        
        if dGd > 1e-10:
            gamma = max(0, min(1, -(dg0 + wGd) / dGd))
        else:
            gamma = 1.0 if (dg0 + wGd) < 0 else 0.0
        
        w = w + gamma * d
    
    # 计算最终梯度
    g_ca = g_avg + torch.matmul(w, g_diff)
    result = g_ca.view_as(grads_list[0])
...
def cagrad_dual_space(grads_family_list, grads_person_list, c=0.5, rescale=1):
    """
    分别对family和person空间应用CAGrad
    
    Args:
        grads_family_list: list of family gradients
        grads_person_list: list of person gradients
        c: CAGrad参数
        rescale: 是否rescale
    
    Returns:
        (family_gradient, person_gradient)
    """
    family_gradient = cagrad(grads_family_list, c=c, rescale=rescale)
    person_gradient = cagrad(grads_person_list, c=c, rescale=rescale)
    
    return family_gradient, person_gradient
...
def multi_scale_guide_function(x, t, grid_person_num, current_grid_distribution_test, grid_control_test,
    ...
    with torch.enable_grad():
        ...
        ## 栅格的男性比例损失
        # male_num = valid_person[:, 2].sum()
        # new_male_ratio_grid = (current_grid_distribution_test[0] * grid_person_num + male_num) / (grid_person_num + valid_person.shape[0])
        ...
        ## 汇总栅格的损失
        grid_loss_total = grid_have_car_person_loss
        ...
        grid_gradient = torch.autograd.grad(outputs=grid_loss_total, inputs=x_family, retain_graph=True)[0]
        ...
        cagrad_c = 1
        grid_gradient = torch.nan_to_num(grid_gradient, nan=0.0)
        district_gradient_family = torch.nan_to_num(district_gradient_family, nan=0.0)
        beijing_gradient_family = torch.nan_to_num(beijing_gradient_family, nan=0.0)
        grid_gradient = grid_gradient / (grid_gradient.norm(dim=-1, keepdim=True) + 1e-8)
        district_gradient_family = district_gradient_family / (district_gradient_family.norm(dim=-1, keepdim=True) + 1e-8)
        beijing_gradient_family = beijing_gradient_family / (beijing_gradient_family.norm(dim=-1, keepdim=True) + 1e-8)

        family_gradient = cagrad(
            [grid_gradient, district_gradient_family, beijing_gradient_family],
            c=cagrad_c,
            rescale=1
        )
        district_gradient_person = torch.nan_to_num(district_gradient_person, nan=0.0)
        beijing_gradient_person = torch.nan_to_num(beijing_gradient_person, nan=0.0)
        district_gradient_person = district_gradient_person / (district_gradient_person.norm(dim=-1, keepdim=True) + 1e-8)
        beijing_gradient_person = beijing_gradient_person / (beijing_gradient_person.norm(dim=-1, keepdim=True) + 1e-8)
        
        person_gradient = cagrad(
            [district_gradient_person, beijing_gradient_person],
            c=cagrad_c,
            rescale=1
        )


        nan_number = (~torch.isfinite(person_gradient)).sum()
        nan_number_family = (~torch.isfinite(family_gradient)).sum()
        if nan_number > 0 or nan_number_family > 0:
            print(f'nan occurred: person={nan_number}, family={nan_number_family}')
            family_gradient = torch.nan_to_num(family_gradient, nan=0.0)
            person_gradient = torch.nan_to_num(person_gradient, nan=0.0)
        if t[0] > 0:
            family_gradient = torch.zeros_like(family_gradient)
            person_gradient = torch.zeros_like(person_gradient)

    return torch.clip(family_gradient, min=-0.5, max=0.5), torch.clip(person_gradient, min=-0.5, max=0.5)
    ...
    for grid in range(start, len(grid_control_district)):
        ...
        for _ in range(10000):
            ...
            sample_family, sample_person, pred_person = diffusion.p_sample_loop(
                ...
                cond_fn=multi_scale_guide_function, model_kwargs=model_kwargs, progress=True, device=device
            )
            samples = sample_family
            samples_person = sample_person

            threshold = (0.5 - 2.38862088) / 0.88397094

            # 生成布尔 mask：只有第 0 列参与判断
            invalid_member_number = samples[:, 0] < threshold

            # 替换第 0 列中 mask 为 True 的元素
            samples[invalid_member_number, 0] = (0.6 - 2.38862088) / 0.88397094

            nan_number = (~torch.isfinite(sample_person)).sum()
            if nan_number > 0:
                print('nan occurred in sample_person')
            ...
            if torch.isnan(current_district_distribution_test).sum()>0:
                print('nan occurred in district distribution update')
        ...
        grid_family.to_csv(f'生成数据_cgrad/人口栅格_{district}_栅格{grid}_家庭数据.csv', index=False)
        grid_person.to_csv(f'生成数据_cgrad/人口栅格_{district}_栅格{grid}_个人数据.csv', index=False)
