...
class PopulationDataEncoder:
    ...
    def __init__(self):
        ...
        self.family_continuous_cols = [
            ...
            '脚踏自行车数量', '电动自行车数量', '摩托车数量', '老年代步车数量'
        ]
        
        # 家庭离散变量
        self.family_categorical_cols = ['have_student', '家庭年收入']
    ...
    def fit_person_data(self, person_df):
        ...
        for col in self.person_categorical_cols:
            ohe = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
            # 处理缺失值
            data_to_fit = person_df[[col]].fillna('unknown').astype(str)
            ohe.fit(data_to_fit)
    ...
    def encode_family(self, family_df):
        ...
        for col in self.family_continuous_cols:
            ...
            encoded_data[f'family_{col}'] = clipped
    ...
    def encode_person(self, person_df):
        ...
        encoded_data['person_age'] = person_df['age'].values * 2 - 1
        
        # 离散变量：one-hot编码
        for col in self.person_categorical_cols:
            data_to_encode = person_df[[col]].fillna('unknown').astype(str)
    ...
    def decode_family_continuous(self, encoded_data):
        ...
        for col in self.family_continuous_cols:
            ...
            standardized_values = encoded_data[f'family_{col}'] * 3
    ...
    def decode_person_continuous(self, encoded_age):
        """解码个人连续变量（年龄）"""
        # 从[-1,1]转换回[0,1]
        normalized_age = (encoded_age + 1) / 2
        
        # 限制在[0,1]范围内
        normalized_age = np.clip(normalized_age, 0, 1)
        
        # 转换回实际年龄（0-123岁）
        actual_age = normalized_age * 123
        
        # 四舍五入为整数年龄
        actual_age = np.round(actual_age).astype(int)
        
        return {
            'age_normalized': normalized_age,
