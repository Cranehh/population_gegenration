...
def compute_person_losses(person_pred, person_true, person_mask, family_pred):
    ...
    losses['person_relation'] = F.cross_entropy(
        person_pred[:, :, 5:11][person_mask],
        person_true[:, :, 5:11][person_mask].argmax(dim=-1),
        reduction="none"
    )
    ...
    losses['person_education'] = F.cross_entropy(
        person_pred[:, :, 11:18][person_mask],
        person_true[:, :, 11:18][person_mask].argmax(dim=-1),
        reduction="none"
    )
    ...
    losses['person_occupation'] = F.cross_entropy(
        person_pred[:, :, 18:27][person_mask],
        person_true[:, :, 18:27][person_mask].argmax(dim=-1),
        reduction="none"
    )
