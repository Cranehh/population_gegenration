...
def main(args):
    ...
    gender_scheduler = DiscreteVariableDiffusion(num_classes=2, num_timesteps=args.num_timesteps, ordered=False, device='cuda')
    license_scheduler = DiscreteVariableDiffusion(num_classes=2, num_timesteps=args.num_timesteps, ordered=False, device='cuda')
    relation_scheduler = DiscreteVariableDiffusion(num_classes=16, num_timesteps=args.num_timesteps, ordered=False,
                                                   device='cuda')
    education_scheduler = DiscreteVariableDiffusion(num_classes=9, num_timesteps=args.num_timesteps, ordered=False,
                                                  device='cuda')
    occupation_scheduler = DiscreteVariableDiffusion(num_classes=20, num_timesteps=args.num_timesteps, ordered=False,
                                                  device='cuda')
    ...
    for epoch in range(args.epochs):
        ...
        for batch in progress_bar:
            ...
            t_person_repeat = t_person.repeat_interleave(8)
            member_data = member_data.view(-1, 51)
            true_member_data = member_data[member_data[:, -1].to(torch.bool)]
            t_person_repeat = t_person_repeat[member_data[:,-1].to(torch.bool)]
            gender_noisy, gender_prob = gender_scheduler.forward_diffusion(true_member_data[:,1:3], t_person_repeat)
            license_noisy, license_prob = license_scheduler.forward_diffusion(true_member_data[:, 3:5], t_person_repeat)
            relation_noisy, relation_prob = relation_scheduler.forward_diffusion(true_member_data[:, 5:21], t_person_repeat)
            education_noisy, edu_prob = education_scheduler.forward_diffusion(true_member_data[:, 21:30], t_person_repeat)
            occupation_noisy, occ_prob = occupation_scheduler.forward_diffusion(true_member_data[:, 30:50], t_person_repeat)

            all_gender_noisy = torch.zeros(member_data.shape[0], 2, device=device)
            all_gender_noisy[member_data[:, -1].to(torch.bool)] = gender_noisy
            all_gender_noisy = all_gender_noisy.view(noise_member.shape[0], noise_member.shape[1], -1)

            all_license_noisy = torch.zeros(member_data.shape[0], 2, device=device)
            all_license_noisy[member_data[:, -1].to(torch.bool)] = license_noisy
            all_license_noisy = all_license_noisy.view(noise_member.shape[0], noise_member.shape[1], -1)

            all_relation_noisy = torch.zeros(member_data.shape[0], 16, device=device)
            all_relation_noisy[member_data[:, -1].to(torch.bool)] = relation_noisy
            all_relation_noisy = all_relation_noisy.view(noise_member.shape[0], noise_member.shape[1], -1)

            all_education_noisy = torch.zeros(member_data.shape[0], 9, device=device)
            all_education_noisy[member_data[:, -1].to(torch.bool)] = education_noisy
            all_education_noisy = all_education_noisy.view(noise_member.shape[0], noise_member.shape[1], -1)

            all_occupation_noisy = torch.zeros(member_data.shape[0], 20, device=device)
            all_occupation_noisy[member_data[:, -1].to(torch.bool)] = occupation_noisy
            all_occupation_noisy = all_occupation_noisy.view(noise_member.shape[0], noise_member.shape[1], -1)

            noisy_member_all = torch.cat((noise_member[:,:,0].view(noise_member.shape[0], noise_member.shape[1], 1),
                                          all_gender_noisy,
                                          all_license_noisy,
                                          all_relation_noisy,
                                          all_education_noisy,
                                          all_occupation_noisy,
                                          noise_member[:,:,-1].view(noise_member.shape[0], noise_member.shape[1], 1)), dim=-1)
            
            x_member_noisy = noisy_member_all
            ...
            member_data = member_data.view(noise_member.shape[0], noise_member.shape[1], -1)
            # 计算损失
